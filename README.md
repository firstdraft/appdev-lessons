...
Here are a few things that need to be changed:
 ...
 1. Begin putting the lesson into sub-lessons by appending `-part-01.md` after `-lesson`. (e.g., `rps-rcav.md` is the original bookdown format, `rps-rcav-lesson.md` is the syntax changes for learn, and `rps-rcav-lesson-part-XX.md` is each part of the lesson which becomes a separate lesson on learn).
